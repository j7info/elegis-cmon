...
- **Tempo mínimo por slide:** Leia `min_seconds` e `unlocked_slides` da URL enviada pelo sistema pai. Slides com índice menor que `unlocked_slides` devem ficar livres para ida e volta. Ao entrar no primeiro slide ainda não registrado, bloqueie apenas a saída para o próximo slide até cumprir `min_seconds`; o sistema pai também registra `INTERACTIVE_SLIDE_START` e valida `INTERACTIVE_SLIDE_END` no backend, para que o histórico não avance antes da hora.
