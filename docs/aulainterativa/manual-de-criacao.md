...
- **Tempo mínimo por slide:** A aula HTML não deve bloquear botões, exibir contagem regressiva nem controlar tempo de permanência. Leia `unlocked_slides` apenas para iniciar o histórico visual já conhecido. A navegação dentro da aula deve ser livre; o sistema pai registra `INTERACTIVE_SLIDE_END`, atualiza o histórico e calcula a presença.
